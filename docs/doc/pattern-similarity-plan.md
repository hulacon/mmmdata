...
Status: **Phases 0–1 complete; Phase 2 fits running; Phases 3–4 submitted** (2026-08-10). Phase 2: SLURM array **46044299**, 6 tasks, compute partition 8 CPU/220 G/24 h — note compute caps at 24 h, not the planned 36 h; TB precedent ran 18.3 h full-grid so masked-2D should fit comfortably. Phases 3–5 code written: `scripts/pattern_similarity/extract_rawtr.py` + `.sbatch` (Phase 3, array **46044912**; smoke-tested on sub-03 TB ses-04 + NAT ses-19 — 61 trials, NAT TR sums match events; has `--verify` mode for the post-hoc checks) and `scripts/pattern_similarity/extract_betas.py` + `.sbatch`. **Phase 4 COMPLETE** (array 46044913, ~30 s/task): all 6 TB caches under `cache/glmsingle/{pipeline}/{sub}/`, 2562 trials each, ROI voxel counts identical to Phase 3 caches (EVC 1401, EAC 604, Hipp 1391, AG 2409, Precuneus 5623, mPFC 976). The NAT branch for Phase 5 handles the masked-2D layout via `brain_mask_index.npy` and is ready — submit with `sbatch --export=ALL,PARADIGM=NAT extract_betas.sbatch` once 46044299 lands and its verification passes. Both caches use fixed atlas ROI voxel sets (out-of-brain-mask voxels NaN) so columns align across runs/pipelines; per-run rawTR NAT windows use rounded per-presentation durations (e.g. "The Bench" 159 TRs vs the 158 chunk figure) — Phase 6's truncate-to-min guard covers this. Earlier: `scripts/pattern_similarity/{shared.py, verify_inputs.py}` (all 37 Task-0 checks pass; qc outputs written), `scripts/glmsingle_natencoding.py` + `.sbatch` (all 6 dry-runs verified: 60 movies, ~3,275 chunk conditions, 115 repeated × 10). **Next**: run `extract_rawtr.py --verify` + Phase 4 verification when 46044912/46044913 finish; write Phase 6 `similarity.py` (can be developed now against the smoke-test caches); when 46044299 finishes, Phase 2 verification then Phase 5.
